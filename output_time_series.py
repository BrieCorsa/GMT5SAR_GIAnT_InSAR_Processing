...
def parse():
    ...
    parser.add_argument('-f', action='store', default='Stack/NSBAS-PARAMS.h5', dest='fname', help='Filename to use. Default: Stack/NSBAS-PARAMS.h5', type=str)
...
if __name__ == '__main__':
    ...
    updated_dates = np.array([dt.date.fromordinal(np.int(i)).strftime("%Y-%m-%d") for i in dates])
#    updated_dates = updated_dates_tmp.strftime("%Y %m %d")
...
#        jj = np.int(np.floor(event.xdata))
	...
	file_out = 'Kilauea_test.txt'	#output file name
	ppd_i = 1/-0.0011111111114	#pixel size (gdalinfo exampleimage.grd)
	ppd_j = 1/0.00111111111112    #pixel size
	center_lat = 19.5   #GPS coordinate
	center_lon = -155.4   #GPS coordinate

	lonmin=-156.066666667  #SAR image corner (gdalinfo exampleimage.grd)
	latmax=20.2888888889	#SAR image corner

	jj = np.int((center_lon-lonmin)*ppd_j)
	ii = np.int((latmax-center_lat)*ppd_i)

	axv.scatter(jj,-ii,s=50,marker='o',facecolors='black',linewidth=0.5,label='GPS')
		...
		output=np.column_stack((updated_dates,dphraw))
		print(output)
